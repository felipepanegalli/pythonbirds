...
class Passaro(Ator):
    ...
    def calcular_posicao(self, tempo):
        ...
        if self._esta_voando():
            t = tempo - self._tempo_de_lancamento
            self._calcular_posicao_vertical(t)
            self._calcular_posicao_horizontal(t)
    ...
    def _calcular_posicao_vertical(self, t):
        angulo_radianos = self._angulo_de_lancamento
        y_atual = self._y_inicial + self.velocidade_escalar * t * math.sin(angulo_radianos)
        y_atual -= (GRAVIDADE * t ** 2) / 2
        self.y = y_atual

    def _calcular_posicao_horizontal(self, t):
        angulo_radianos = self._angulo_de_lancamento
        x_atual = self._x_inicial + self.velocidade_escalar * t * math.cos(angulo_radianos)
        self.x = x_atual
